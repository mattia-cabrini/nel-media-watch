...
# INSTALLED binary on that target through daemon(8): the job is fully
...
if [ ! -x "$INSTALLED_BINARY" ]; then
    echo "ERROR: $INSTALLED_BINARY not found: run 'make install' first" >&2
    exit 1
fi

# The detached job runs the INSTALLED binary: refuse to launch it when
# it differs from the one just built from the repository ('make run'
# builds first), otherwise the job would silently run outdated code.
if ! cmp -s "$SOURCE_DIRECTORY/$BUILT_BINARY" "$INSTALLED_BINARY"; then
    echo "ERROR: installed '$RUNTIME_BINARY' differs from the repository build: run 'make install' first" >&2
    exit 1
fi
...
daemon -c -f "$INSTALLED_BINARY" run "$TARGET_NAME" || exit 1
